...
![image-20210818104022689](JVM.assets/image-20210818104022689.png)
...
- 正常代码一句一句解释执行
- 热点代码即使编译器编译了以后缓存, 后面可以直接执行机器码无需解释

![image-20210818104550802](JVM.assets/image-20210818104550802.png)
...
![image-20210818135748292](JVM.assets/image-20210818135748292.png)
...
### 魔数(0xCAFEBABE（咖啡宝贝？）(1-4) magic number
...
### Class文件的版本号(5-8) minor version major version
...
- 描述符
  ...
  - 根据描述符规则，基本数据类型（byte、char、double、float、int、long、short、boolean）以及代表无返回值的void类型都用一个大写V字符来表示，
  - 而对象类型则用字符L加对象的全限定名
...
- 在**虚拟机栈**（栈帧中的本地变量表）中引用的对象，譬如各个线程被调用的方法堆栈中使用到的**参数、局部变量、临时变量**等。
...
- 强分代假说（Strong Generational Hypothesis) ：熬过越多次垃圾收集过程的对象就越难以消亡。
  ...
  - Java堆划分出不同的区域之后，垃圾收集器才可以每次只回收其中某一个或者某些部分的区域——因而才有了“Minor GC”“Major GC”“Full GC”这样的回收类型的划分；
...
- A的引用(孩子结点)全部标记完成 是黑色
- B遍历了但是还没来得及标记D(D本来是B的孩子结点), 但是B->D引用消失
- 此时A增加了到D的引用
- 问题
  - B->D的引用消失, 所以重新标记的时候没有办法访问D
  - A已经是黑色, 不会再从A访问D因为默认A的孩子结点已经全部访问过了, 但是实际上还是有A->D的引用
  - 这时, D因为没有办法被访问导致重新标记结束仍然是白色所以被当成垃圾回收了, 但是实际上还有A的引用指向D的

![image-20210818142459322](JVM.assets/image-20210818142459322.png)



#### 解决方案

- Incremental Update (CMS的方案)
  - 将A标记为灰色
- 仍然存在漏洞所以remark阶段仍然需要从头扫描一遍

![image-20210818143212554](JVM.assets/image-20210818143212554.png)
...
- 我们要解决并发扫描时的对象消失问题，只需破坏这两个条件的任意一个即可。由此分别产生了两种解决方案：
  ...
  - **原始快照** (Snapshot At the Begining SATB)要破坏的是第二个条件，当灰色对象要删除指向白色对象的引用关系时，就将这个要删除的引用记录下来，在并发扫描结束之后，再将这些记录过的引用关系中的灰色对象为根，重新扫描一次。这也可以简化理解为，无论引用关系删除与否，都会按照刚刚开始扫描那一刻的对象图快照来进行搜索。
...
- 可能存在的问题
  1. 并发标记阶段,对象被引用, 此时未被标记为垃圾, 但是并发标记结束, 这个对象不再被引用, 其实是垃圾但是并发标记结束的时候并没有标记为垃圾, 是浮动垃圾
  2. 并发标记阶段,对象没有被引用, 是垃圾, 但是标记为垃圾以后, 又有引用指向这个对象, 所以并发标记结束, 这个对象不是垃圾却因为并发标记时没有被引用后来才被引用导致被标为垃圾
- 优点
  - 并发收集、低停顿
- 缺点
  - **对处理器资源非常敏感**
...
![image-20210818164341182](JVM.assets/image-20210818164341182.png)

- 基于Region的内存布局形式, Region是Java将堆划分为多个大小相等的独立区域, 最多可以有2048个Region
- 提供并发的类卸载的支持
- 希望做出一款能够建立起“停顿时间模型”（PausePrediction Model）的收集器，停顿时间模型的意思是能够支持指定在一个长度为M毫秒的时间片段内，消耗在垃圾收集上的时间大概率不超过N毫秒这样的目标
- Young GC
  - 不会在Eden区放满了马上就进行YoungGC, 而是会计算Eden区回收所需的时间, 
  - 如果远远小于 -XX:MaxGCPauseMillis设定的值, 增加年轻代的的region继续给新对象存放, 直到下一次Eden区满了, 
  - 如果回收时间接近  -XX:MaxGCPauseMillis 就会触发YoungGC
- Mixed GC模式
  - 老年代堆的占有率达到参数 (-XX:InitiatingHeapOccupancyPercent) 设定的值触发, 回收所有Young, 部分Old(根据期望的GC停顿时间确定Old区域垃圾回收优先顺序), 以及大对象区域
...
- CMS VS G1
  ...
  - 与CMS的“标记-清除”算法不同，G1从整体来看是基于“**标记-整理**”算法实现的收集器，但从局部（两个Region之间）上看又是基于“**标记-复制**”算法实现，无论如何，这两种算法都意味着**G1运作期间不会产生内存空间碎片**，垃圾收集完成之后能提供规整的可用内存。这种特性有利于程序长时间运行，在程序为大对象分配内存时不容易因无法找到连续内存空间而提前触发下一次收集
...
![image-20210818170436284](JVM.assets/image-20210818170436284.png)



![image-20210818174430223](JVM.assets/image-20210818174430223.png)
