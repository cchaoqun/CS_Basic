...
- 断线重连后的从服务器向主服务器发送PSYNC命令, 拔高自己当前的复制偏移量
...
- 字典键是被监视主服务器的名字
- 字典的值是被监视主服务器对弈的sentinel.c/sentinelRedisInstance结构
...
- 如果一个实例在down_after_milliseconds毫秒内持续向Sentinel返回无效回复, nameSentinel修改这个实例的flags属性, 打开SRI_S_DOWN标识, 标识实例进入主观下线
...
客户端向节点发送与数据库键有关的命令时, 接收命令的结点静思园命令要处理的数据库键属于哪个槽, 检查这个槽是否指派给自己
...
重新分片将任意数量已经指派给某个结点的槽该位指派给另一个结点, 并且相关槽所属的键值对都从源结点移动到目标结点
...
- 结点成为从节点 复制 主节点 这个信息变成消磁发送给集群其他节点
...
- 开始故障转移, 集群配置纪元的值自增1
- 对于每个配置纪元, 集群每个负责处理槽的主节点有一次投票的机会, 会投给第一个要求自己投票的从结点
- 从节点发现正在复制的结点下线, 从结点会向集群广播一条 CLUSTERMSG_TYPE_FAILOVER_AUTH_REQUEST消息, 要求所有收到消息的有投票权的主节点向这个从节点投票
- 拥有投票权的主节点会向第一个向自己发送消息的结点投票, 投完不能再投
- 当一个从节点收到了集群中N个主节点中的 至少N/2+1 就成为了新的主节点
- 每个配置纪元, 每个主节点只有一次投票机会, 所以最多一个从节点称为主节点
- 没有从节点收到足够的票, 集群进入新的配置纪元继续选举直到选出主节点
- 这个选取Sentinel领头相似都是基于Raft算法
