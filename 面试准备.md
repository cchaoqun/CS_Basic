...
![捕获](C:/Users/Chaoq/Desktop/捕获.PNG)
...
9.29一面（20min左右） 
  ...
  [项目]()介绍，负责内容，数据集哪里来的，数据集规模，有哪些优化改进，[项目]()做到什么程度了 
  ...
  [算法]()知识：归并[排序]()的原理和时间复杂度，是不是稳定的，什么原因 
  ...
  手撕代码：两个大整数相加（写完之后面试官说要考虑负数和进制，哪怕抛出个提示呢？草率了） 

  智力题：20个球，有一个异常的，给你个天平，最多多少次可以找到它（经小提示想到的） 

  第一步，分成4个5，两次称重能找到是哪个5异常，且能知道是重异常、轻异常 

  第二步，假设是重异常。然后把5分成2,2,1，先称重2个2，如果平衡，则找到异常；否则到重的2个球中找 


  第三步，拿一个正常小球和2个球中的其中一个相比，如果平衡，异常为另一个，如果不平衡，异常为重的那一个
...
https://note.youdao.com/ynoteshare/index.html?id=0fc1cf2282731c0e1948a70bb1ca02b0&type=note&_time=1634024612464

10/8 二面
自我介绍
实验室论文 [项目](https://www.nowcoder.com/jump/super-jump/word?word=项目) 公司实习[项目](https://www.nowcoder.com/jump/super-jump/word?word=项目) 问的很细
无八股
智力题 1-100 有条件猜数字
手撕 第三大数字

Spring Springmvc Springboot在[项目](https://www.nowcoder.com/jump/super-jump/word?word=项目)中的使用
分布式事务
笔试时候的一道题 n个数字，求倒数第几位数
两个无环[链表](https://www.nowcoder.com/jump/super-jump/word?word=链表)，判断是否有交点



作者：牛客570548252号
链接：https://www.nowcoder.com/discuss/762390?type=2&order=3&pos=21&page=1&source_id=discuss_tag_nctrack&channel=-1
来源：牛客网
...
一面提问：
 ...
 (1)[项目]()提问，[项目]()用到了内网穿透，解释内网穿透流程

 (2)[算法]()分析：100w个数字如何选取10个最大的

 (3)进程和线程区别是什么，进城通信的方式有哪些，什么是死锁

 (4)如何用udp协议构建一个通信系统，使用udp的协议说一个

 (5)讲一下你熟悉的linux命令，如何查看某个cpu的状态

 (6)解释一下非对称加密和对称加密的区别，为什么https能防止中间人攻击
 ...
 (2)讲一下大学期间做的[项目]()，面试官对[项目]()内容提问

 (3)智力题：一百瓶药水里有一瓶毒药，老鼠喝了毒药会死，至少需要几只老鼠才能判断出毒药

 (4)什么是内存泄漏，如何快速定位到内存泄漏位置
...
非科班出身，有哪些计算机基础和自己学习的知识？ 

解释非阻塞 Socket ，监听 Socket 是阻塞还是非阻塞？ 
...
50亿个非负整数，实现[排序]()和剔除重复值？（面试官说用 位图 ）
...
一面： 
  ...
  3.代码：给定一个整数数组nums和一个正整数k，找出是否有可能把这个数分成k个非空子集，其总和都相等。 

  4.java中的泛型是什么，泛型是如何实现的？ 
...
[项目]()相关，问了之前的比赛，其实那个比赛有点混，于是借机让面试官问最近的[项目]()（有个涉及到HTTP网络传输的问题，跟面试官有点没讲请，于是跳过了）

问了有哪些[排序]()[算法]()

快排怎么实现，时间复杂度，最坏情况时间复杂度和情况
...
LeetCode最近做题吗？总共做了多少题？

#### 二面 40min

智力题，分盐

编程题，将数组分成k个相等的子数组（面试官一直在引导，但是确实当时没想清楚，后面时间不够没写完



[算法](https://www.nowcoder.com/jump/super-jump/word?word=算法)，一个字符串，能改其中k个字符，求改完之后的字符串中最长重复子串。此时我人已经懵逼了，面试官提示了很多我就是没思路，直接结束面试了。
...
java[项目]()，java[项目]()是开源的，有什么改进吗？

 内存泄漏

 操作系统，进程通信方式，最重要的是问有什么区别，答不上来去区别
...
https://www.jianshu.com/p/00d67a878001
...
\5. jvm垃圾收集  参数设置  如何打印线程堆栈信息 性能调优做过么(没做过)
