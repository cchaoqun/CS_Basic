...
Good Morning Interviewer, I am ChaoqunCheng and I am currently a graduate student in University of Pennsylvania
...
- 自我介绍
- 问了java分布式商城项目, 问的很深, 自己做的有点拉胯, 没答好
  - 介绍一下你这个项目用了哪些框架, 分了哪些模块
  - 网关模块的功能是什么, 用的哪个组件
  - RPC调用的协议是什么
  - 除了HTTP还知道哪些?
- 面试官应该挺有水平的
...
## 微软 一面 11/01 10点 45 min

- 自我介绍
- 为什么转行
- 算法题 [354. 俄罗斯套娃信封问题](https://leetcode-cn.com/problems/russian-doll-envelopes/)
  - 根据第一个数字排序从小到大, 相同的根据第二个数字从大到小排列
  - 遍历, 根据第二个数字求LIS 最长上升子序列长度
  - follow up 如果自己写排序, 用什么? 快排
  - 实现以下二维的快排
...
## 微软 二面 11/03 10点 45 min

- 自我介绍
- 算法题 找到给定二叉搜索树给定结点按照中序遍历的下一个结点, 每个结点可以有left right parent三个指针分别指向左右子节点和父节点
  - 判断结点的右子节点是否为空
  - 右子节点存在, 找到右子树的最左子节点
  - 右子节点不存在, 向上找父节点(父节点如果为祖父结点的右子节点, 继续向上找, 直到找到一个祖父结点的左子节点为结点的父节点, 返回这个祖父结点
  - 不存在这个祖父结点返回null
...
public class MS2 {
    ...
    public Node findNext(Node node){
        // 左叶子节点 ==> 父节点 parent
        // 右叶子节点 ==> 父节点是祖父结点的左子节点 parent.parent
        //              父节点是祖父节点的右子节点 null
        // 非叶子节点 ==> 右子树的最左子节点
        //              右子节点为null 父节点
        ...
        if(node.right==null){
            if(node.parent==null){
                ...
                return findHighestLeftParent(node);
            }
        }else{
            //右子树的最左子节点
            return findRightLeft(node.right);
        }
    ...
    public Node findHighestLeftParent(Node node){
        while(node!=null && node.parent.right.val==node.val){
            node = node.parent;
        }
...
public static void main(String[] args) {
        ...
        String str3 = "abcde##";
        String str4 = "##ab";
        boolean res1 = isEqual(str1.toCharArray(), str2.toCharArray());
        boolean res2 = isEqual(str3.toCharArray(), str4.toCharArray());
        boolean res3 = isEqual(str2.toCharArray(), str3.toCharArray());
//        System.out.println("res1==res2?:"+res1);
        System.out.println("res3==res4?:"+res2);
//        System.out.println("res2==res3?:"+res3);
    }
	/**
     * 字符串
     *  # 表示退格键
...
public void test(){
    int[] coins = {1,2,5};
    int target  = 8;
    int cnt = coinChange(coins, target);
...
public int coinChange(int[] coins, int target){
    ...
    backtrack(coins, index, target, res, path);
...
public void backtrack(int[] coins, int index, int target, List<List<Integer>> res, List<Integer> path){
    //有效解
    if(target==0){
    ...
    //选 coins[i]
    if(target>0 && target>=coins[index]){
        path.add(coins[index]);
        backtrack(coins, index, target-coins[index], res, path);
        path.remove(path.size()-1);
    }
    //不选 coins[i]
    backtrack(coins, index+1, target, res, path);
}
```
...
- 操作系统
  - 进程包含哪些?
  - 一个进程的线程a打开了进程的文件读了50个字节, 另一个线程b这时候打开同一个文件会接着读还是从头读?
    - 接着读
  - CPU轮转调度, 存在一个恢复现场的过程, 能说一下吗?
    - 线程aCPU 时间片用完, 系统调用
    - 线程的a的寄存器数据压栈进入线程a的内核站
    - 根据CPU的调度规则选择线程b运行
    - TCB变成线程b的TCB
    - 线程b的内核站poll出寄存器的之前的值存储到寄存器中
    - 返回用户态, 切换到了线程b的用户栈
  - 文件系统了解吗?
    - 硬链接软连接 inode
    - 硬链接不创建新的inode 文件内容指向目的文件
    - inode是文件磁盘块的信息, 创建时间, 上一次修改时间, 所有者
- 数据库
  - MySQL了解哪些?
    - 存储引擎, 索引, mvcc, 缓存, 锁, 事务
  - 说一下B+树的性质
  - B+树查询的时间复杂度是多少? 比二分查找快吗?
    - 比二分慢, 因为每一层之间还存在一个遍历的过程找到下一层的分支在哪
- 网络
  - 交换机是那一层?
    - 链路层, 路由器是网路层
  - tcp是哪一层的协议
...
- 算法题
  - dfs+根据搜索树的性质判断是否需要遍历左右子树

```java
//给一颗二叉搜索树, 希望找到树中所有在[start, end]区间的结点值, 按升序返回
...
class Main {
	...
	查询区间范围
	*/
	public List<Integer> findRegion(Node root, int start, int end){
		if(root==null){
			return null;
		}
		List<Integer> res = new ArrayList<>();
		dfs(root, start, end, res);
		return res;
	}
	...
	public void dfs(Node root, int l, int r, List<Integer> res){
		...
		}else if(root.val<=r){// l<= x <= r
			dfs(root.left, l, r, res);
			res.add(root.val);
			dfs(root.right, l, r, res);
		}else if(root.val>r){// x> r
			dfs(root.left, l, r, res);
		}
	}
