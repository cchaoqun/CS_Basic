...
- 面试官您好, 我是chaoqun 应聘后端开发工程师 目前是Upen在读研究生
- 技术栈主要是Java, 看过一些JDK源码, 了解Java并发, JVM的相关的知识点
- 计算机基础方面 热爱算法与数据结构, 很喜欢解决算法问题, 自己也尝试实现过一些数据结构, 比如堆, AVL树, 红黑树, 熟悉操作系统的进程管理内存管理
- 对于计算机网络, 了解对于TCP/IP模型中各层的一些主要协议
- 熟悉MySQL 索引,mvcc原理 
- 熟悉Redis的线程模型, 以及分布式场景下缓存穿透, 击穿, 雪崩问题下的解决方法, 
- 项目方面主要实现了一个基于Raft算法的KV存储系统, 提供Put/Append/Get操作, 在Raft算法的基础上做了一些该进实现了数据库的线性一致性
  - 收获了对于分布式场景下, 学习了如何解决复制所带来的数据一致性问题, 
  - 同时对于Raft算法这个广泛应用于分布式架构下的各个组件中的算法有了更深的理解
  - 比如redis的哨兵模式选举局部领头以及redis主从复制模式下, 选出从节点代替主节点的过程都应用了raft算法的选主思想
- 对于开发常用的框架有一定的了解, Spring IoC Aop 的原理, SpringMVC工作流程, SpringBoot的自动配置的原理
- 了解分布式锁, 分布式限流
...
2. 深入理解数据库,缓存,框架, 中间件技术的原理, 
   1. 一方面了解了原理才能在做项目技术选型时有自己的观点, 直到为什么要选这个, 优缺点在于哪些地方
...
- 挑战1
  - 需要考虑分布式系统中可能会发生的各种边界条件, 并且提出对应的处理方案
    ...
    - 比如发生网路分区的故障, 可能存在闹裂的现象, 一个时间存在多个主节点去处理客户端的请求导致数据不一致
